# Tarea 2 · Relación de THD con productos/servicios, sostenibilidad e investigación

## 🏢 Empresa seleccionada
Abengoa, S.A. Multinacional con sede en Sevilla especializada en infraestructuras sostenibles, energía renovable, agua y biocombustibles.
Aunque ha tenido dificultades financieras, su experiencia y tecnología en energía solar, desalación y operación de plantas es muy relevante.

---

## 🧩 Relación entre THD y productos/servicios
Explica cómo la empresa podría usar cada tecnología para crear o mejorar un servicio digital.
Debes basarte en una aplicación real encontrada en noticias o informes.

| Tecnología | Producto/servicio digital | Caso real encontrado (enlace) | Beneficio para el cliente | Beneficio para la empresa |
|-----------|----------------------------|-------------------------------|----------------------------|---------------------------|
| IA | Mantenimiento predictivo en plantas solares y de agua | Abengoa reporta mantenimiento predictivo en sus plantas O&M usando técnicas | Mayor fiabilidad del suministro de energía o agua, menos cortes | energía o agua, menos cortes. Reduce costes operativos, optimiza intervenciones y alarga la vida útil de los activos. |
| IoT | Sensores en tanques de agua | Infraestructura de Abengoa en agua y energía: Abengoa diseña plantas de agua y energía renovable | Monitorización en tiempo real de parámetros críticos | Permite control remoto, detectar fugas o anomalías e intervenir antes de que sean graves |
| Big Data | Plataforma de analítica para datos operacionales | Servicios O&M de Abengoa incluyen “data analysis and treatment in a single platform” para salud de activo | Clientes pueden tener reportes transparentes de eficiencia y estado | Mejora la toma de decisiones operativas, optimiza recursos y reduce riesgo. |
| Cloud Computing | Infraestructura cloud para gestión global de datos | Abengoa tiene múltiples plantas en distintos países y usa sistemas de información para gestión E-O&M | Acceso a dashboards e informes desde cualquier lugar, para clientes institucionales | Escalabilidad, integración de datos de diferentes plantas y localizaciones, recuperación ante desastres |
| Blockchain | Trazabilidad en la cadena de suministro de biomasa y agua | Podría usarse para trazabilidad de biocombustibles o recursos hídricos | Más transparencia | Permite certificaciones verdes, mejora la reputación y puede abrir nuevos mercados sostenibles |
| Ciberseguridad | Protección de plataforma de analítica para datos operacionales | | | |
| RA/RV | Simulador AR / VR para entrenamiento técnico en plantas solares | No he encontrado información para este caso | | |

---

## 🌱 Impacto en la economía sostenible y eficiente
Describe cómo cada tecnología contribuye a la sostenibilidad y la eficiencia.

| Tecnología | Ahorro energético | Reducción de residuos | Mejora operativa | Riesgos/limitaciones |
|-----------|-------------------|-----------------------|------------------|----------------------|
| IA | Optimiza mantenimiento | Alargar vida útil de componentes reduce necesidad de recambios | Ahorro en costes | Modelos costosos |
| IoT | Monitorización en tiempo real permite operar con más eficiencia | Detección temprana de fugas | Mejor control de operación | Seguridad en dispositivos |
| Big Data | Analítica de datos operativos puede optimizar consumo, producción y recursos | Permite decisiones informadas para reducir desperdicios | Toma de decisiones más eficiente | Privacidad y gobernanza de datos |
| Cloud Computing | Menos servidores físicos locales | Centralización facilita actualizaciones y mejoras sin duplicar hardware | Escalabilidad | Dependencia de proveedor cloud |
| Blockchain | Facilita mercados | Registro inmutable de origen de materia prima y uso de agua | Confianza con personas otros grupos | Consumo |
| Ciberseguridad | Evita interrupciones por ataques que podrían generar despilfarro energético en reinicios | Protege activos valiosos | Continuidad operativa | Coste elevado |
| RA/RV | Formación virtual reduce desplazamientos físicos para entrenar personal | Menos piezas dañadas durante entrenamiento | Formación más rápida, segura | Necesidad de hardware AR/VR |

---

## 🔍 Investigación de casos reales (obligatoria)
Resume brevemente los casos y explica cómo podrían aplicarse a tu empresa.

- Caso IA:
- Caso IoT:
- Caso Big Data:
- Caso Cloud:
- Caso Blockchain:
- Caso Ciberseguridad:
- Caso RA/RV:

---

## 🧠 Reflexión final
- ¿Qué tecnología aporta más valor a los productos/servicios?
- ¿Cuál genera mayor impacto sostenible?
- ¿Qué combinación sería más eficiente para la empresa?

## 📚 Fuentes consultadas
(Enlaces a las noticias o informes usados)



